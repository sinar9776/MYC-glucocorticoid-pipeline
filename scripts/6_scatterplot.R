...
plot_xy <- function(df, xcol, ycol, cat_col, cols, title, xlab, ylab, lim, legend_title) {
  ggplot(df, aes(x = .data[[xcol]], y = .data[[ycol]])) +
    ...
    geom_point(data = df[df[[cat_col]] != "Other", ],
               aes(color = .data[[cat_col]]), size = pt_cat, alpha = 0.9) +
...
## Part 3 — x = drug/DMSO, y = (drug+OHT)/DMSO
axis_y_p3 <- function(drug)
  bquote(bold(atop(.(drug) ~ "+ OHT / DMSO", "(" * log[2] * "FC" * ")")))

## ============================================================
## PART 1) x = drug/DMSO,  y = (drug+OHT)/(DMSO+OHT)
## ============================================================
...
make_scatter_df <- function(drug) {
  ...
  full_join(
    t_no  %>% dplyr::select(feature_id, log2FoldChange) %>% dplyr::rename(x = log2FoldChange),
    t_yes %>% dplyr::select(feature_id, log2FoldChange) %>% dplyr::rename(y = log2FoldChange),
    by = "feature_id"
  )
}
...
for (drug in drug_list) {
  ...
  p1 <- plot_xy(df, "x", "y", "main",  cols_main,  drug, axis_x(drug), axis_y_p1(drug), scatter_lim, "Category")
  p2 <- plot_xy(df, "x", "y", "ind",   cols_ind,   drug, axis_x(drug), axis_y_p1(drug), scatter_lim, "Independent")
  p3 <- plot_xy(df, "x", "y", "shift", cols_shift, drug, axis_x(drug), axis_y_p1(drug), scatter_lim, "Shifted")
  p4 <- plot_xy(df, "x", "y", "add",   cols_add,   drug, axis_x(drug), axis_y_p1(drug), scatter_lim, "Additive")
...
## ============================================================
## PART 2) x = OHT/DMSO,  y = (drug+OHT)/DMSO
## ============================================================
...
corr_make_df <- function(drug) {
  ...
  full_join(
    t_x %>% dplyr::select(feature_id, log2FoldChange) %>% dplyr::rename(x = log2FoldChange),
    t_y %>% dplyr::select(feature_id, log2FoldChange) %>% dplyr::rename(y = log2FoldChange),
    by = "feature_id"
  )
}
...
p3_make_df <- function(drug) {
  t_x <- load_tt(drug)                              # drug/DMSO
  t_y <- load_tt(paste0(drug, "_OHT_vs_DMSO"))      # (drug+OHT)/DMSO
  id_x <- pick_id_col(t_x)
  id_y <- pick_id_col(t_y)
  t_x <- dplyr::rename(t_x, feature_id = !!id_x)
  t_y <- dplyr::rename(t_y, feature_id = !!id_y)
...
p3_list <- lapply(drug_list, p3_make_df)
names(p3_list) <- drug_list
p3_lim <- c(-10, 10)
...
for (drug in drug_list) {
  ...
  p1 <- plot_xy(df, "x", "y", "main",  cols_main,  drug, axis_x(drug), axis_y_p3(drug), p3_lim, "Category")
  p2 <- plot_xy(df, "x", "y", "ind",   cols_ind,   drug, axis_x(drug), axis_y_p3(drug), p3_lim, "Independent")
  p3 <- plot_xy(df, "x", "y", "shift", cols_shift, drug, axis_x(drug), axis_y_p3(drug), p3_lim, "Shifted")
  p4 <- plot_xy(df, "x", "y", "add",   cols_add,   drug, axis_x(drug), axis_y_p3(drug), p3_lim, "Additive")
  
  save_both(file.path(fig_out, paste0("DRUG_vs_DRUGOHT_", drug)),
            p1 + theme(legend.position = "none"), 6, 6)
  save_both(file.path(fig_out, paste0("DRUG_vs_DRUGOHT_", drug, "_independent")),
            p2 + theme(legend.position = "none"), 6, 6)
  save_both(file.path(fig_out, paste0("DRUG_vs_DRUGOHT_", drug, "_shifted")),
            p3 + theme(legend.position = "none"), 6, 6)
  save_both(file.path(fig_out, paste0("DRUG_vs_DRUGOHT_", drug, "_additive")),
            p4 + theme(legend.position = "none"), 6, 6)
  
  p3_main_list[[drug]]  <- p1
  p3_ind_list[[drug]]   <- p2
  p3_shift_list[[drug]] <- p3
  p3_add_list[[drug]]   <- p4
}

save_both(file.path(fig_out, "DRUG_vs_DRUGOHT_ALL_main"),
          wrap_plots(p3_main_list,  ncol = 3) & theme(legend.position = "none"), 18, 12)
save_both(file.path(fig_out, "DRUG_vs_DRUGOHT_ALL_independent"),
          wrap_plots(p3_ind_list,   ncol = 3) & theme(legend.position = "none"), 18, 12)
save_both(file.path(fig_out, "DRUG_vs_DRUGOHT_ALL_shifted"),
          wrap_plots(p3_shift_list, ncol = 3) & theme(legend.position = "none"), 18, 12)
save_both(file.path(fig_out, "DRUG_vs_DRUGOHT_ALL_additive"),
          wrap_plots(p3_add_list,   ncol = 3) & theme(legend.position = "none"), 18, 12)

p3_legend_types <- list(
  list(p3_main_list[[1]],  "Category",    "DRUG_vs_DRUGOHT_LEGEND_main"),
  list(p3_ind_list[[1]],   "Independent", "DRUG_vs_DRUGOHT_LEGEND_independent"),
  list(p3_shift_list[[1]], "Shifted",     "DRUG_vs_DRUGOHT_LEGEND_shifted"),
  list(p3_add_list[[1]],   "Additive",    "DRUG_vs_DRUGOHT_LEGEND_additive")
)

for (lg in p3_legend_types) {
  p_leg <- make_vertical_legend(lg[[1]], lg[[2]])
  leg   <- get_legend(p_leg)
  if (!is.null(leg))
    save_both(file.path(fig_out, lg[[3]]), patchwork::wrap_elements(full = leg), 4, 8)
