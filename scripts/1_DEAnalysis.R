...
stopifnot(all(c("SampleID", "group", "Batch") %in% colnames(metadata)))
...
for (g in levels(dds1$group)) {
  ...
  sub_metadata$Batch <- droplevels(sub_metadata$Batch)
  ## use Batch in design only if more than one level present in subset
  design_sub <- if (nlevels(sub_metadata$Batch) > 1) ~ Batch + group else ~ group
  dds_sub <- DESeqDataSetFromMatrix(round(sub_counts), sub_metadata, design = design_sub)
...
for (drug in drug_groups) {
  ...
  dds_tmp <- DESeq(dds_tmp, parallel = FALSE)
